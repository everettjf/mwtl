...
public:
    void BuildUI() override {
        ...
        ui.Add(subtitle_, L"Modern dialogs, clipboard, drag and drop, and persistent placement in one native window.");
        ...
        SetLayout(mwtl::Column().Margin(24.0_dip).Gap(10.0_dip)
            .Add(title_, mwtl::Fixed(34.0_dip)).Add(subtitle_, mwtl::Fixed(24.0_dip))
            .Add(dialogs_, mwtl::Fixed(28.0_dip))
            .Add(mwtl::Row().Gap(8.0_dip).Add(open_, mwtl::Fixed(240.0_dip)).Add(save_, mwtl::Fixed(240.0_dip)).Add(folder_, mwtl::Fixed(240.0_dip)).Add(mwtl::Column(), mwtl::Stretch()), mwtl::Fixed(44.0_dip))
            .Add(clipboard_, mwtl::Fixed(28.0_dip))
            .Add(mwtl::Row().Gap(8.0_dip).Add(copy_, mwtl::Fixed(240.0_dip)).Add(paste_, mwtl::Fixed(240.0_dip)).Add(task_, mwtl::Fixed(240.0_dip)).Add(mwtl::Column(), mwtl::Stretch()), mwtl::Fixed(44.0_dip))
            .Add(drop_zone_, mwtl::Fixed(76.0_dip))
            .Add(path_label_, mwtl::Auto()).Add(path_, mwtl::Fixed(34.0_dip))
            .Add(activity_label_, mwtl::Auto()).Add(activity_, mwtl::Stretch()));

        mwtl::SavedWindowPlacement saved;
        if (mwtl::LoadWindowPlacementFromRegistry(HKEY_CURRENT_USER, kRegistryKey, L"Window", saved))
    ...
    mwtl::EventResult OnCommand(const mwtl::CommandEvent& event) override {
        ...
        if (event.IsClicked(folder_)) {
            ShowResult(L"Folder", mwtl::ShowFolderDialog({GetHwnd(), L"Choose a workspace folder"}));
            return mwtl::EventResult::Handled();
        }
        if (event.IsClicked(copy_)) {
            const auto value = path_.GetText();
            AppendActivity(mwtl::SetClipboardText(GetHwnd(), value) ? L"Copied the latest result." : L"Clipboard copy failed.");
            return mwtl::EventResult::Handled();
        }
        ...
        if (event.IsClicked(task_)) {
            mwtl::ShowTaskDialog(GetHwnd(), L"mwtl desktop integration", L"Native Windows shell features", L"These helpers preserve explicit ownership and report cancellation separately from failure.");
...
private:
    void ShowResult(std::wstring_view action, const mwtl::FileDialogResult& result) {
        ...
        } else AppendActivity(std::wstring(action) + (result.Cancelled() ? L": cancelled." : L": failed."));
    }
    ...
    mwtl::Button open_, save_, folder_, copy_, paste_, task_;
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int show) {
    return mwtl::RunApplication<DesktopIntegrationWindow>(instance, show, {.title = L"Desktop integration", .initial_bounds = {{}, {1120.0_dip, 720.0_dip}}, .use_default_bounds = false});
}
