...
int main() {
    ...
    if (!left_top || left_top->monitor != 0 || left_top->corner != Corner::top_left ||
        ...
        interior || !seam_left || seam_left->monitor != 0 ||
        seam_left->corner != Corner::top_right || !seam_right ||
        seam_right->monitor != 1 || seam_right->corner != Corner::top_left ||
        outside_half_open) return EXIT_FAILURE;
