...
bool Tooltip::AddTool(HWND tool, std::wstring_view text) { if (window_ == nullptr || tool == nullptr || ::GetParent(tool) != owner_) return false; texts_.push_back(std::make_unique<std::wstring>(text)); TOOLINFOW info{sizeof(info)}; info.uFlags = TTF_IDISHWND | TTF_SUBCLASS; info.hwnd = owner_; info.uId = reinterpret_cast<UINT_PTR>(tool); info.lpszText = texts_.back()->data(); if (::SendMessageW(window_, TTM_ADDTOOLW, 0, reinterpret_cast<LPARAM>(&info)) == FALSE) { texts_.pop_back(); return false; } return true; }
